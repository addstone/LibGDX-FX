...
final public class LwjglFXInput implements Input {
    ...
    @Override public boolean isKeyJustPressed(int i) {
        return false;
    }

    @Override public void getTextInput(TextInputListener textInputListener, String s, String s1, String s2) {
    ...
    @Override public boolean isCatchBackKey() {
        return false;
    }
    ...
    void processEvents() {
        ...
        if (!isPressed && hasFocus && target.getScene().getRoot().isPressed()) {
            hasFocus = false;
        }
        synchronized (this) {
            if (processor != null) {
